// [    --- <!-- Data --!> ---    ] \\
const line_pic = "https://media.discordapp.net/attachments/798926497490010112/798926565554913290/2021-45-13_04__45__50.png?width=615&height=230";
const down_vote = "👎";
const up_vote = "👍";
const error = "❌";
const timeing = "⏱";
...
client.on("message", async (msg) => {
  if (msg.content.startsWith(prefix + "public")) {
    ...
    msg.channel.send(new Discord.MessageEmbed().setTitle("**Bot Orders**").setThumbnail(msg.member.user.avatarURL({ format: "gif", format: "png", dynamic: true, size: 1024 }))
      ...
      .addField(prefix + "sug", "add a suggest", true)
      .setFooter(`need more info? just type ${prefix}help [cmd name]`)
    )
                cooldown_command.add(msg.author.id);
            setTimeout(() => {
...
client.on("message", async message => {
  ...
  if (message.channel.id != line_id) return;
  if(message.author.id === client.user.id) return;
      if (message.author.send) {
    message.channel.send({files: [`${line_pic}`]});
...
if(message.author.bot)return;
const sugch = message.channel.id === sug_room
...
// <!-- colors --!> \\
client.on("message", m999 => {
  let args = m999.content.split(" ").slice(1).join(" ");
  if (m999.content.startsWith(prefix + "createcolors")) {
    if (msg.author.bot) return;
    if (msg.channel.type == "dm") return msg.channel.send(new Discord.MessageEmbed().setColor("RED").setDescription(error + ` **You Can't Use This Command In DM's!**`).setFooter(`Request By ${msg.author.tag}`).setTimestamp())
    if (!args) return m999.channel.send("`يرجي اختيار كم لون `");
    if (!m999.member.hasPermission("MANAGE_ROLES"))
      return m999.channel.send("`**⚠ | `[MANAGE_ROLES]` لا يوجد لديك صلاحية**");
    m999.channel.send(`**${success} |Created __${args}__ Colors**`);
    setInterval(function() { });
    let count = 0;
    let ecount = 0;
    for (let x = 1; x < `${parseInt(args) + 1}`; x++) {
      m999.guild.roles.create({ data: { name: x, color: "RANDOM" } });
    }
  }
});
